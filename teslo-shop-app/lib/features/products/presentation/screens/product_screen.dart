      ...
      child: Scaffold(
        ...
        floatingActionButton: FloatingActionButton(
          ...
          onPressed: () {
            ...
            final isPostingData =
                ref.read(productFormProvider(productState.product!)).isPosting;

            if (isPostingData) return;
...
class _ProductInformation extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Padding(
      ...
      child: Column(
        key: const Key('productInformation'),
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('Generales'),
          const SizedBox(height: 15),
          CustomProductField(
            key: const Key('productTitleField'),
            isTopField: true,
            label: 'Título',
            initialValue: productForm.title.value,
            onChanged:
                ref.read(productFormProvider(product).notifier).onTitleChanged,
            errorMessage: productForm.isFormPosted
                ? productForm.title.errorMessage
                : null,
          ),
          CustomProductField(
            key: const Key('productSlugField'),
            label: 'Slug',
            initialValue: productForm.slug.value,
            onChanged:
                ref.read(productFormProvider(product).notifier).onSlugChanged,
            errorMessage:
                productForm.isFormPosted ? productForm.slug.errorMessage : null,
          ),
          CustomProductField(
            ...
            errorMessage: productForm.isFormPosted
                ? productForm.price.errorMessage
                : null,
          ),
          const SizedBox(height: 15),
          const Text('Extras'),
          _SizeSelector(
            key: const Key('productSizeSelector'),
          ...
          _GenderSelector(
            key: const Key('productGenderSelector'),
          ...
          const SizedBox(height: 15),
          CustomProductField(
            key: const Key('productStockField'),
            isTopField: true,
            label: 'Existencias',
            keyboardType: const TextInputType.numberWithOptions(decimal: true),
            ...
            errorMessage: productForm.isFormPosted
                ? productForm.inStock.errorMessage
                : null,
          ),
          CustomProductField(
          ...
          CustomProductField(
            key: const Key('productTagsField'),
            isBottomField: true,
            maxLines: 2,
